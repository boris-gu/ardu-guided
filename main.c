...
void change_tx_period(DWORD *last_time, DWORD *period, int new_period);
...
DWORD tx_mav_period = PERIOD_TX_START_TASK_MS;
DWORD tx_mav_last_time = 0;
    ...
    switch (current_task) {
      case TASK_START_TASK: {
        ...
        if (drone_pose.time_boot_ms != 0) {
          ...
          change_tx_period(&tx_mav_last_time, &tx_mav_period, PERIOD_TX_TO_GUIDED_MS);
      ...
      case TASK_TO_GUIDED: {
        ...
        if (drone_hbeat.custom_mode == COPTER_MODE_LOITER) {
          ...
          change_tx_period(&tx_mav_last_time, &tx_mav_period, PERIOD_TX_DO_NOTHING_MS);
        } else if (drone_hbeat.custom_mode == COPTER_MODE_GUIDED) {
          current_task = TASK_ARM;
          change_tx_period(&tx_mav_last_time, &tx_mav_period, PERIOD_TX_ARM_MS);
          printf("-- SM: TASK_ARM\n");
        }
        break;
      }
      case TASK_ARM: {
        ...
        if (drone_hbeat.custom_mode != COPTER_MODE_GUIDED) {
          ...
          change_tx_period(&tx_mav_last_time, &tx_mav_period, PERIOD_TX_TAKEOFF_MS);
      ...
      case TASK_TAKEOFF: {
        if (drone_hbeat.custom_mode != COPTER_MODE_GUIDED) {
          ...
          change_tx_period(&tx_mav_last_time, &tx_mav_period, PERIOD_TX_DO_NOTHING_MS);
           ...
           change_tx_period(&tx_mav_last_time, &tx_mav_period, PERIOD_TX_FLY_TO_COORD_MS);
...
void* mavlink_tx_thread(void* arg) {
  ...
  for (;;) {
    ...
    if (current_time - tx_mav_last_time >= tx_mav_period) {
      tx_mav_last_time = current_time;
      if (drone_id) {
        switch (current_task) {
...
void change_tx_period(DWORD *last_time, DWORD *period,int new_period) {
  *last_time = 0; // Сброс для быстрого перехода к следующему состоянию
  *period = new_period;
}
